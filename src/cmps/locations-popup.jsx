...
export function LocationsPopUp({ history, closeAllModals ,onSetLocationPopUp,onSetFilter}) {
    function onPushTo(pushTo,loc) {
        closeAllModals()
    ...
    return (
        <div className="location-modal flex column">
            {locations.map((location, idx) => <div key={idx} className="location-card flex align-center" onClick={() => { onPushTo(`explore/?location=${location.city}`,location.city) }}>
